@php
    ...
    $firstCbtNo = is_array($soalCbt) && !empty($soalCbt) ? array_key_first($soalCbt) : 1;
...
<div id="ruangUjianApp" 
     ...
     x-data="{
        ...
        cbtKeys: {{ json_encode(collect($soalCbt ?? [])->keys()->all()) }},
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden transition-all duration-200">
        ...
        <div class="px-5 py-3.5 bg-slate-50/70 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 text-xs">
            ...
            <div class="flex items-center gap-2.5">
                @if(!$adaFormOnlineDikerjakan)
                    @if($isHasilAkhirTersedia)
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11px] font-bold">
                            ...
                            <span>Hasil Asesmen Resmi Terbit</span>
                        </span>
                    ...
                    @else
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200 text-[11px] font-bold">
                            <span class="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
                            <span>Asesmen Tatap Muka di TUK</span>
                        </span>
                    @endif
                ...
                @if(!$adaFormOnlineDikerjakan)
                    @if($isHasilAkhirTersedia)
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-semibold">
                            ...
                            <span>Keputusan Resmi Terbit</span>
                        </span>
                    @else
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200 text-xs font-medium">
                            ...
                            <span>Observasi Langsung di TUK</span>
                        </span>
                    @endif
                @elseif($bisaAksesUjian && !$isReadonly)
                    <!-- Indikator Autosave -->
        ...
        @if($adaFormOnlineDikerjakan && ($bisaAksesUjian || $isSubmitted || ($statusJadwalSesi === 'selesai')))
            <!-- Tab Navigasi Segmented (Smooth & Sleek - Hanya Tampil Saat Sesi Dimulai / Selesai dan Ada Formulir Online) -->
    ...
    @if($adaFormOnlineDikerjakan && $isSubmitted)
        <!-- =========================================================================
             BANNER NOTIFIKASI UJIAN TELAH DIKUMPULKAN & JAWABAN TERKUNCI (HANYA UNTUK FORM ONLINE)
             ========================================================================= -->
    ...
    @elseif(!$adaFormOnlineDikerjakan)
        <!-- =========================================================================
             SKEMA ASESMEN LANGSUNG DI TUK (TANPA FORM ONLINE ASESI)
             ========================================================================= -->
        @if($isHasilAkhirTersedia && $rekomendasi)
            <!-- KARTU HASIL AKHIR ASESMEN RESMI -->
            <div class="bg-white rounded-2xl border {{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'border-emerald-200' : 'border-amber-200' }} shadow-sm overflow-hidden p-6 sm:p-8 space-y-6">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                    <div class="flex items-center gap-3.5">
                        <div class="w-12 h-12 rounded-2xl {{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'bg-emerald-500 text-white' : 'bg-amber-500 text-white' }} flex items-center justify-center font-bold text-xl shadow-xs">
                            @if(strtolower($rekomendasi->keputusan ?? '') === 'kompeten')
                        ...
                        <div>
                            <span class="text-[11px] font-extrabold uppercase tracking-wider text-slate-400">Keputusan Resmi Asesor Penguji</span>
                            <h3 class="text-xl font-extrabold text-slate-900 flex items-center gap-2">
                                <span>{{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'KOMPETEN (K)' : 'BELUM KOMPETEN (BK)' }}</span>
                            </h3>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 self-start sm:self-center">
                        <span class="px-3.5 py-1.5 rounded-xl font-bold text-xs {{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-800 border border-amber-200' }}">
                            Status: Selesai Diases
                        </span>
                    </div>
                </div>
                ...
                <div class="p-4 sm:p-5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-2">
                    <span class="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                        ...
                        <span>Catatan Rekomendasi & Umpan Balik Asesor:</span>
                    </span>
                    <p class="text-xs sm:text-sm text-slate-800 italic leading-relaxed">
                        "{{ $rekomendasi->catatan_rekomendasi ?? 'Asesi telah menunjukkan kinerja dan pemahaman yang baik dalam seluruh tahapan uji kompetensi.' }}"
                    </p>
                </div>

                <!-- Grid Metadata Penilaian -->
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                    <div class="p-3.5 bg-white border border-slate-200 rounded-xl">
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Asesor Penguji:</span>
                        <strong class="text-slate-800 font-bold mt-0.5 block truncate" title="{{ $rekomendasi->asesor?->nama_lengkap ?? $pendaftaran->asesor?->nama_lengkap ?? 'Asesor LSP' }}">{{ $rekomendasi->asesor?->nama_lengkap ?? $pendaftaran->asesor?->nama_lengkap ?? 'Asesor LSP' }}</strong>
                    </div>
                    <div class="p-3.5 bg-white border border-slate-200 rounded-xl">
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Tanggal Keputusan:</span>
                        <strong class="text-slate-800 font-bold mt-0.5 block">{{ !empty($rekomendasi->tanggal_rekomendasi) ? \Carbon\Carbon::parse($rekomendasi->tanggal_rekomendasi)->translatedFormat('d F Y') : '-' }}</strong>
                    </div>
                    <div class="p-3.5 bg-white border border-slate-200 rounded-xl">
                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Metode Pelaksanaan:</span>
                        <strong class="text-slate-800 font-bold mt-0.5 block">Observasi Demonstrasi & Wawancara di TUK</strong>
                    </div>
                </div>

                <!-- Tanda Tangan Asesor jika ada -->
                @if(!empty($rekomendasi->tanda_tangan_asesor))
                    <div class="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-100">
                        <div class="text-xs text-slate-500">
                            <span class="block font-semibold text-slate-700">Telah Disahkan oleh Asesor Penguji:</span>
                            <span class="text-[11px]">Tanda tangan digital tercatat resmi pada sistem LSP SMKN 1 Gunungputri</span>
                        </div>
                        <div class="bg-white p-2 border border-slate-200 rounded-xl shadow-2xs">
                            <img src="{{ $rekomendasi->tanda_tangan_asesor }}" alt="Tanda Tangan Asesor" class="h-12 max-w-[150px] object-contain">
                        </div>
                    </div>
                @endif

                <div class="pt-2 flex flex-wrap items-center gap-3">
                    <a href="{{ route('asesi.hasil-nilai') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-xs transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        <span>Lihat Rincian Penilaian & Dokumen (FR.AK.02)</span>
                    </a>
                </div>
            </div>
        @else
            <!-- KARTU ASESMEN PRAKTIK LANGSUNG DI TUK (MENUNGGU PELAKSANAAN & PENILAIAN ASESOR) -->
            <div class="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-6 sm:p-8 space-y-6">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                    <div>
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-blue-800 border border-blue-200 text-xs font-bold mb-2">
                            <span class="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
                            <span>Asesmen Tatap Muka di Tempat Uji Kompetensi (TUK)</span>
                        </div>
                        <h3 class="text-base sm:text-lg font-bold text-slate-900">
                            Pelaksanaan Asesmen Praktik Langsung & Observasi di TUK
                        </h3>
                        <p class="text-xs text-slate-500 mt-0.5">
                            Skema sertifikasi: <strong class="text-slate-700">{{ $pendaftaran->skema->nama_skema ?? '-' }} ({{ $pendaftaran->skema->kode_skema ?? '-' }})</strong>
                        </p>
                    </div>
                    <div class="shrink-0">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 text-amber-800 border border-amber-200 text-xs font-bold shadow-2xs">
                            ...
                            <span>Menunggu Pelaksanaan & Penilaian Asesor</span>
                        </span>
                    </div>
                </div>

                <!-- Banner Informasi: Tidak Ada Form Online -->
                <div class="p-4 sm:p-5 rounded-2xl bg-blue-50/70 border border-blue-200 text-blue-950 flex items-start gap-3.5">
                    <div class="w-9 h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center shrink-0 shadow-2xs mt-0.5">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
                    <div class="space-y-1 text-xs">
                        <h4 class="font-bold text-sm text-blue-950">Informasi Penting Pelaksanaan Uji Kompetensi</h4>
                        <p class="text-blue-900 leading-relaxed">
                            Pada skema sertifikasi ini, seluruh proses asesmen dilaksanakan secara <strong>langsung (tatap muka / demonstrasi praktik)</strong> bersama Asesor Penguji di Tempat Uji Kompetensi (TUK). <strong>Tidak ada lembar formulir ujian online yang perlu Anda isi atau kumpulkan di sistem ini.</strong>
                        </p>
                        <p class="text-blue-800 text-[11px]">
                            Seluruh lembar observasi demonstrasi (FR.IA.01/FR.IA.02) dan pertanyaan lisan (FR.IA.03) dinilai dan diisi langsung oleh Asesor Penguji saat sesi asesmen berlangsung di TUK.
                        </p>
                    </div>
                </div>

                <!-- Ringkasan Jadwal & Lokasi Pelaksanaan -->
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                    <div class="p-4 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Tempat Uji (TUK):</span>
                        <strong class="text-slate-800 text-xs block truncate" title="{{ $tukFormatted }}">{{ $tukFormatted }}</strong>
                        <span class="text-[10px] text-slate-500 block">Ruang Pelaksanaan Uji</span>
                    </div>
                    <div class="p-4 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Tanggal Ujian:</span>
                        <strong class="text-slate-800 text-xs block">{{ $tglUjianFormatted }}</strong>
                        <span class="text-[10px] text-slate-500 block">Hari Pelaksanaan</span>
                    </div>
                    <div class="p-4 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Waktu Sesi:</span>
                        <strong class="text-blue-700 font-mono font-bold text-xs block">{{ $jamMulaiFormatted }} - {{ $jamSelesaiFormatted }}</strong>
                        <span class="text-[10px] text-slate-500 block">Waktu Indonesia Barat</span>
                    </div>
                    <div class="p-4 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Asesor Penguji:</span>
                        <strong class="text-slate-800 text-xs block truncate" title="{{ $asesorNama }}">{{ $asesorNama }}</strong>
                        <span class="text-[10px] text-slate-500 block">Penguji Kompetensi Resmi</span>
                    </div>
                </div>

                <!-- Panduan Bagi Asesi Saat Mengikuti Asesmen di TUK -->
                <div class="p-5 rounded-2xl bg-white border border-slate-200/80 space-y-3 text-xs">
                    <h4 class="font-bold text-slate-800 flex items-center gap-2">
                        ...
                        <span>Petunjuk Pelaksanaan Bagi Peserta (Asesi) di TUK:</span>
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50/70 border border-slate-200/60">
                            <span class="w-5 h-5 rounded-full bg-blue-100 text-blue-700 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">1</span>
                            <div>
                                <strong class="text-slate-800 block text-xs">Hadir Tepat Waktu</strong>
                                <span class="text-slate-600 text-[11px]">Hadir di lokasi TUK sekurang-kurangnya 15 menit sebelum sesi asesmen dimulai.</span>
                            </div>
                        </div>
                        <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50/70 border border-slate-200/60">
                            <span class="w-5 h-5 rounded-full bg-blue-100 text-blue-700 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">2</span>
                            <div>
                                <strong class="text-slate-800 block text-xs">Membawa Identitas Resmi</strong>
                                <span class="text-slate-600 text-[11px]">Bawa KTP/Kartu Pelajar serta bukti pendaftaran asesmen yang telah disetujui.</span>
                            </div>
                        </div>
                        <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50/70 border border-slate-200/60">
                            <span class="w-5 h-5 rounded-full bg-blue-100 text-blue-700 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">3</span>
                            <div>
                                <strong class="text-slate-800 block text-xs">Patuhi Tata Tertib & K3</strong>
                                <span class="text-slate-600 text-[11px]">Gunakan pakaian rapi/seragam dan patuhi standar Keselamatan dan Kesehatan Kerja (K3) di tempat uji.</span>
                            </div>
                        </div>
                        <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50/70 border border-slate-200/60">
                            <span class="w-5 h-5 rounded-full bg-blue-100 text-blue-700 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">4</span>
                            <div>
                                <strong class="text-slate-800 block text-xs">Demonstrasikan Keterampilan Kerja</strong>
                                <span class="text-slate-600 text-[11px]">Laksanakan instruksi demonstrasi praktik dan jawab pertanyaan klarifikasi dari Asesor Penguji dengan percaya diri.</span>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Status Penilaian & Informasi Hasil Akhir -->
                <div class="p-4 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between gap-3 text-xs text-slate-600">
                    <div class="flex items-center gap-2">
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 bg-amber-500"></span>
                        </span>
                        <span class="text-[11px]">Hasil rekomendasi akhir uji kompetensi (<strong class="text-slate-800">Kompeten / Belum Kompeten</strong>) akan dipublikasikan di halaman ini setelah Asesor menyelesaikan penilaian resmi.</span>
                    </div>
                    <button type="button" @click="window.location.reload()" class="px-3 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-100 font-semibold text-xs text-slate-700 shrink-0 cursor-pointer shadow-2xs">
                        Periksa Hasil
                    </button>
                </div>
            </div>
    ...
    @elseif($isSesiBelumMulai)
        <!-- =========================================================================
             LAYAR TUNGGU JADWAL ASESMEN ONLINE: ASESMEN BELUM DIMULAI SESUAI JADWAL
             ========================================================================= -->
            ...
            <div class="max-w-2xl mx-auto bg-slate-50/80 border border-slate-200/80 rounded-2xl p-4 sm:p-5 text-left text-xs space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    ...
                    <div>
                        ...
                        <strong class="text-blue-700 font-mono font-bold text-xs">{{ $jamMulaiFormatted }} - {{ $jamSelesaiFormatted }}</strong>
                    </div>
    ...
    @else
        <!-- =========================================================================
             SESI ASESMEN FORM ONLINE AKTIF: LEMBAR PENGERJAAN FR.IA
             ========================================================================= -->
        @if($isHasilAkhirTersedia && $rekomendasi)
            <!-- Kartu Hasil Akhir Asesmen juga tampil jika ujian online sudah selesai dinilai -->
            <div class="bg-white rounded-2xl border {{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'border-emerald-200' : 'border-amber-200' }} shadow-sm overflow-hidden p-6 sm:p-7 space-y-4">
                <div class="flex items-center justify-between gap-3 border-b border-slate-100 pb-3">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl {{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'bg-emerald-600 text-white' : 'bg-amber-600 text-white' }} flex items-center justify-center font-bold">
                            @if(strtolower($rekomendasi->keputusan ?? '') === 'kompeten')
                                ✓
                            @else
                                !
                        ...
                        <div>
                            <span class="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Hasil Akhir Asesmen</span>
                            <h4 class="font-extrabold text-base text-slate-900">{{ strtolower($rekomendasi->keputusan ?? '') === 'kompeten' ? 'KOMPETEN (K)' : 'BELUM KOMPETEN (BK)' }}</h4>
                        </div>
                    </div>
                    <a href="{{ route('asesi.hasil-nilai') }}" class="px-3.5 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 font-bold text-xs rounded-xl border border-blue-200 transition">
                        Rincian Penilaian &rarr;
                    </a>
                </div>
                <p class="text-xs text-slate-700 italic">"{{ $rekomendasi->catatan_rekomendasi ?? 'Asesmen telah selesai dilaksanakan.' }}"</p>
            </div>
        @endif
    ...
    <!-- =========================================================================
         ...
         ========================================================================= -->
    @if(!$isReadonly && $adaFormOnlineDikerjakan)
